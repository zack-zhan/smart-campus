...
export default {
      ...
      if (parent.staffList && parent.staffList.length > 0) {
        parent.staffList.map(x => {
          ...
          if (this.rightDataKeys.findIndex(y => x.staffId === y) > -1) {
            this.$store.commit('editBulletingroupTeacher', staff)
          } else {
  ...
  mounted () {
    console.log('123')
    this.rightDataKeys = this.$store.state.bulletingroup.editTeacher.map(x => x.key)
    getdepartmentandstafflist().then(res => {
      if (res.code === 200) {
        this.originDepartmentList = res.body
  ...
  computed: {
    rightDataSource () {
      return this.$store.state.bulletingroup.editTeacher
    }
  }
}
</script>
