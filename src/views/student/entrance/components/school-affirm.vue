<template>
  <div class="box">
    ...
    <div class="btn-bar">
      <a-button @click="entrance">确认入学</a-button>
    </div>
    <div class="table">
        ...
        >
          <template slot="operation" slot-scope="text">
            <div class="operation">
              ...
              <!-- <span @click="edit(text)" v-if="text.entranceStr === '拒绝入学'">重置</span> -->
            </div>
          </template>
        </a-table>
      </a-locale-provider>
...
<script>
import { originstudentpage, confirmEntrance } from '@/api'
...
export default {
  ...
  methods: {
    ...
    // 确认入学
    entrance () {
      confirmEntrance(this.selectedKeys)
        .then(res => {
          if (res.code === 200) {
            this.$message.success('入学成功')
            this.list()
            this.selectedKeys = []
          } else {
            this.$message.error(res.message)
    ...
    // 查看
    look (row) {
      this.$router.push({path: '/entranceLook', query: {id: row.originStudentid, entranceStats: 3}})
    },
    // 编辑
    edit (row) {
