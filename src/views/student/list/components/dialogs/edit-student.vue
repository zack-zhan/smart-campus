  ...
  <div class="dialogBox">
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item label="证件类型：" prop="certificateType">
        <el-select v-model="ruleForm.certificateTypeStr" placeholder="请选择">
      ...
      <el-form-item label="状态：" prop="status">
        <el-select v-model="ruleForm.statusStr" placeholder="请选择">
      ...
      <el-form-item label="宿舍：" v-if="dormitoryShow">
        <span class="select" v-if="ruleForm.dormitoryName">{{ruleForm.dormitoryName}}</span>
        <span class="add" @click="editSuperiorDialogVisible = true">编辑添加</span>
        <a-modal :footer="null" :visible="editSuperiorDialogVisible" :closable="false" :bodyStyle="bodyStyle">
          <editSuperior @onConfirm="onDormitoryConfirm" @close='editSuperiorDialogVisible = false'/>
        </a-modal>
      </el-form-item>
...
<script>
import { studentone, gradelist, classlistbygradeid, studentupdate, aliOSSSignature } from '@/api'
import editSuperior from './edit-dormitory'
...
export default {
  ...
  data () {
    return {
      ...
      editSuperiorDialogVisible: false,
      bodyStyle: {
        'padding': '24px 0 0 0'
      },
      dormitoryId: '',
  ...
  methods: {
    ...
    // 获取详情
    list () {
      studentone(this.ruleForm.id)
        .then(res => {
          console.log(res)
          if (res.code === 200) {
            this.ruleForm = res.body
            this.ruleForm.gender = this.ruleForm.gender.toString()
            this.ruleForm.lodgingFlag = this.ruleForm.lodgingFlag.toString()
            this.gradeId = res.body.gradeId
    ...
    onDormitoryConfirm (selectArr) {
      ...
      this.ruleForm.dormitoryName = selectArr.name
      this.dormitoryId = selectArr.id
      this.editSuperiorDialogVisible = false
    },
    // 日期时间转成字符串方法
    formatDate (date) {
    ...
    sava (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (this.ruleForm.certificateTypeStr === '居民身份证') {
            certificateType = 0
          } else if (this.ruleForm.certificateTypeStr === '护照') {
            certificateType = 1
          } else if (this.ruleForm.certificateTypeStr === '回乡证') {
            certificateType = 2
          } else if (this.ruleForm.certificateTypeStr === '台胞证') {
            certificateType = 3
          } else if (this.ruleForm.certificateTypeStr === '其他') {
            certificateType = 4
          } else {
            certificateType = this.ruleForm.certificateTypeStr
          }
          let status = null
          if (this.ruleForm.statusStr === '待入学') {
            status = 0
          } else if (this.ruleForm.statusStr === '已入学') {
            status = 1
          } else if (this.ruleForm.statusStr === '已离校') {
            status = 2
          } else if (this.ruleForm.statusStr === '休学') {
            status = 3
          } else {
          ...
          const obj = {
            ...
            studentCode: this.ruleForm.studentCode,
            gender: parseInt(this.ruleForm.gender),
            certificateType: certificateType,
            certificateNo: this.ruleForm.certificateNo,
            nation: this.ruleForm.nation,
            birthday: this.formatDate(new Date(Date.parse(this.ruleForm.birthday))),
            lodgingFlag: parseInt(this.ruleForm.lodgingFlag),
            status: status,
            schoolDate: this.formatDate(new Date(Date.parse(this.ruleForm.schoolDate))),
