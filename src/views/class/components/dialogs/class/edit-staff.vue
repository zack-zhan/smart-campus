          ...
          <el-scrollbar style="height:100%;">
            <div v-for="(item,index) in searchData" :key="index">
              <el-checkbox v-model="item.checked"
                          :disabled="$store.state.grade.editClassDirector.staffid && $store.state.grade.editClassDirector.staffid !== item.staffid"
...
export default {
  ...
  methods: {
        ...
        .then(res => {
          ...
          if (res.code === 200) {
            ...
            this.staffList.map(item => {
              if (item.staffid === this.$store.state.grade.editClassDirector.staffid) {
                item.checked = true
              }
            })
          } else {
    ...
    clickCheck (index) {
      ...
      if (item.checked) {
        this.$store.commit('setEditClassDirector', item)
        this.select = item
      } else {
        this.$store.commit('setEditClassDirector', {})
      }
    },
    // 确认
    save () {
