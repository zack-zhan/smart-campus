<template>
  <div class="dialogBox">
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      <el-form-item label="年级名称:" prop="gradeName">
        <el-input v-model="ruleForm.gradeName" placeholder="输入名称"></el-input>
      </el-form-item>
      <el-form-item label="年级别名:" prop="aliasName">
        <el-input v-model="ruleForm.aliasName" placeholder="输入名称"></el-input>
      </el-form-item>
      <el-form-item label="年级主任:" prop="leaderCode" class="ts">
        <span class="select" v-if="ruleForm.leaderCode.name">{{ruleForm.leaderCode.name}}</span>
        <span class="add" @click="addStaff">编辑添加</span>
      </el-form-item>
    </el-form>
    <div class="footer">
      <el-button class="btn" type="primary" @click="save('ruleForm')">保存</el-button>
      <el-button class="btn" @click="cancel">取消</el-button>
    </div>
  </div>
</template>

<script>
import eventBus from './eventBus.js' // 事件车
import { gradecreate } from '@/api'
export default {
  data () {
    return {
      // 添加年级表单
      ruleForm: {
        gradeName: '',
        aliasName: '',
        leaderCode: ''
      },
      // 表单验证
      rules: {
        gradeName: [
          { required: true, message: '请输入年级名称', trigger: 'blur' },
          { pattern: /^[\u4e00-\u9fa5_a-zA-Z0-9]+$/, message: '不允许输入特殊字符' }
        ],
        aliasName: [
          { required: true, message: '请输入年级别名', trigger: 'blur' },
          { pattern: /^[\u4e00-\u9fa5_a-zA-Z0-9]+$/, message: '不允许输入特殊字符' }
        ],
        leaderCode: [
          { required: true, message: '请选择', trigger: 'blur' }
        ]
      }
    }
  },
  methods: {
    // 添加年班主任
    addStaff () {
      this.$emit('addGradeStaff')
    },
    // 选中的值
    select (select) {
      eventBus.$on('select', (select) => {
        this.ruleForm.leaderCode = select
      })
    },
    // 保存
    save (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const obj = {
            gradeName: this.ruleForm.gradeName,
            aliasName: this.ruleForm.aliasName,
            leaderCode: this.ruleForm.leaderCode.staffid
          }
          gradecreate(obj)
            .then(res => {
              if (res.code === 200) {
                this.$message.success('保存成功')
                this.$emit('success')

                this.ruleForm.gradeName = ''
                this.ruleForm.aliasName = ''
                this.ruleForm.leaderCode = ''
                this.$store.commit('setAddGradeDirector', {})
              } else {
                this.$message.error(res.message)
              }
            })
        } else {
          console.log('error submit!!')
          return false
        }
      })
    },
    // 点击取消发送关闭方法
    cancel () {
      this.ruleForm.gradeName = ''
      this.ruleForm.aliasName = ''
      this.ruleForm.leaderCode = ''
      this.$emit('close')

      this.$store.commit('setAddGradeDirector', {})
    }
  },
  mounted () {
    this.select()
  }
}
</script>

<style lang="scss" scoped>
.dialogBox{
  padding: 0 30px;
}
.footer{
  padding-left: 100px;
}
.btn{
  width:65px;
  height:32px;
  line-height: 32px;
  padding: 0;
}
.add{
  display: inline-block;
  font-size:14px;
  font-family:PingFangSC-Regular;
  font-weight:400;
  color:rgba(24,144,255,1);
  cursor:pointer;
}
.select{
  display: inline-block;
  padding: 0 5px;
  height:22px;
  line-height: 22px;
  margin-right: 5px;
  background:rgba(245,245,245,1);
  border-radius:4px;
  border:1px solid rgba(217,217,217,1);
}
</style>

<style scoped>
.dialogBox >>> .el-form-item__content{
  width: 400px;
  line-height: 32px;
}
.dialogBox >>> .el-form-item{
  margin-bottom: 30px;
}
.dialogBox >>> .el-form-item__label{
  line-height: 32px;
}
.dialogBox >>> .el-input__inner{
  height: 32px;
  line-height: 32px;
}
.dialogBox >>> .el-input__icon{
  line-height: 32px;
}
.dialogBox >>> .el-input--suffix{
  width: 400px;
}
.ts >>> .el-input--suffix{
  width: 197px;
  float: left;
}
</style>
