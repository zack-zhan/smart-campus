          ...
          <el-scrollbar style="height:100%;">
            <div v-for="(item,index) in searchData" :key="index">
              <el-checkbox v-model="item.checked"
                          :disabled="$store.state.grade.editGradeDirector.staffid && $store.state.grade.editGradeDirector.staffid !== item.staffid"
...
export default {
  ...
  methods: {
        ...
        .then(res => {
          ...
          if (res.code === 200) {
            ...
            this.staffList.map(item => {
              if (item.staffid === this.$store.state.grade.editGradeDirector.staffid) {
                item.checked = true
              }
            })
          } else {
    ...
    clickCheck (index) {
      ...
      if (item.checked) {
        this.$store.commit('setEditGradeDirector', item)
        this.select = item
      } else {
        this.$store.commit('setEditGradeDirector', {})
      }
    },
    // 确认
    save () {
