        ...
        <div class="tree">
          <el-scrollbar style="height:100%;">
            <div v-for="(item,index) in searchData" :key="index" style="margin-left:16px;">
              <el-checkbox v-model="item.checked" @change="clickCheck(index)" :disabled="item.checked!=true&&chooseIndex!='reset'" v-if="item.isLast === 3"></el-checkbox>
...
export default {
  ...
  methods: {
    ...
    // 确认
    save () {
      this.$emit('onConfirm', this.selectArr)
    },
    // 点击取消发送关闭方法
    cancel () {
      this.$emit('close')
